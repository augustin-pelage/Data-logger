...
from PyQt5.QtWidgets import QApplication, QMainWindow, QMessageBox, QDialog, QFileDialog, QPushButton, QLabel, QSpinBox, QWidget
from PyQt5.QtNetwork import QAbstractSocket, QTcpSocket
#import personnal_vna
...
class FigureTab:
  ...
  def plot_imp(self):
    ...
    max = np.maximum(0.01, data1.max())
    self.plot_curves(freq, data1, '|Z|, \u03A9', (0.9, 3.1), data2, r'$\angle$ Z, deg', (-198, 198))
...
##########Association data logger#########################
    #self.data_logger.clicked.connect(self.show_data_logger)
##########################################################
    ...
    """def show_data_logger(self):
      new_data_logger = personnal_vna.Data_logger()
      new_data_logger.show_data_logger()"""

    """ import Redpitaya_interface
      from Redpitaya_interface import MyWindow
...
########Signal la fin de l'acquisition au data logger##################
        """from personnal_vna import Data_logger
        new_data_logger = Data_logger()
        new_data_logger.repet_afer_complet()"""
#######################################################################
  ...
  def impedance(self, freq):
    ...
    if len(self.external_open) > 0 and isinstance(open, np.ndarray):
        open = self.external_open
    if len(self.external_short) > 0 and isinstance(short, np.ndarray):
        short = self.external_short
    if len(self.external_load) > 0 and isinstance(load, np.ndarray):
        load = self.external_load
